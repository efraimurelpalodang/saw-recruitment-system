...
export default async function SeleksiLayout({
  children,
}: {
    ...
    <div className="flex h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      <div className="w-72 border-r bg-muted/20 flex flex-col h-full">
        <div className="p-4 border-b">
          <h3 className="font-semibold text-lg">Lowongan Ditutup</h3>
          <p className="text-sm text-muted-foreground">Pilih lowongan untuk seleksi</p>
        </div>
          ...
          {closedJobs.length === 0 ? (
            ...
            <div className="flex flex-col">
              {closedJobs.map((job) => (
                <Link
                  key={job.id}
                  href={`/dashboard/hrd/seleksi/${job.id}`}
